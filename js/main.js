...
function getAllPossibleCoordsBishop(pieceCoord) {
    ...
    for (var i = pieceCoord.i + 1; i < 8 && j >= 0; j-- , i++) {
        var coord = { i: i, j: j };
        if (!isEmptyCell(coord)) break;
        res.push(coord);
    }
...
function getAllPossibleCoordsKnight(pieceCoord) {
    ...
    for (var i = pieceCoord.i - 2; i <= (pieceCoord.i + 2); i++) {
        for (var j = pieceCoord.j - 2; j <= (pieceCoord.j + 2); j++) {
            var coord = { i: i, j: j };
            if (Math.abs(i - pieceCoord.i) === 2 && Math.abs(j - pieceCoord.j) === 1 &&
                i >= 0 && j >= 0 && i < 8 && j < 8) {
            ...
            if (Math.abs(i - pieceCoord.i) === 1 && Math.abs(j - pieceCoord.j) === 2 &&
                i >= 0 && j >= 0 && i < 8 && j < 8) {
                if (isEmptyCell(coord)) {
                    res.push(coord);
